import { setup, assign, stateIn, and, not, type SnapshotFrom } from 'xstate';
...
export const savageMachine = setup({
  ...
  guards: {
    ...
    healToZero: ({ context, event }) =>
      Math.max(0, context.wounds - asHeal(event).amount) === 0,
                ...
                unshaken: {
                  on: {
                    TAKE_DAMAGE: [
                      ...
                      { guard: and(['exceedsMax', 'incapCritFail']), target: '#savage.dead', actions: ['setWoundsToMax'] },
                      { guard: and(['exceedsMax', 'incapFail']), target: '#savage.alive.damageTrack.incapacitated.bleedingOut', actions: ['setWoundsToMax'] },
                      { guard: and(['exceedsMax', 'incapSuccess']), target: '#savage.alive.damageTrack.incapacitated.stable', actions: ['setWoundsToMax'] },
                ...
                shaken: {
                  on: {
                    TAKE_DAMAGE: [
                      ...
                      { guard: and(['exceedsMaxShaken', 'incapCritFail']), target: '#savage.dead', actions: ['setWoundsToMax'] },
                      { guard: and(['exceedsMaxShaken', 'incapFail']), target: '#savage.alive.damageTrack.incapacitated.bleedingOut', actions: ['setWoundsToMax'] },
                      { guard: and(['exceedsMaxShaken', 'incapSuccess']), target: '#savage.alive.damageTrack.incapacitated.stable', actions: ['setWoundsToMax'] },
...
export function isActive(snap: SavageSnapshot): boolean {
  return (
    snap.matches({ alive: { damageTrack: 'active' } }) &&
...
export function totalPenalty(snap: SavageSnapshot): number {
  ...
  const fatigue =
    snap.matches({ alive: { fatigueTrack: 'exhausted' } }) || snap.matches({ alive: { fatigueTrack: 'incapByFatigue' } }) ? 2
    : snap.matches({ alive: { fatigueTrack: 'fatigued' } }) ? 1
    : 0;
  return -(wp + fatigue);
}
